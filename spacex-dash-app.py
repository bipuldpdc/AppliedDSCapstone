...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        counts_df = pd.DataFrame({'Class': counts.index, 'Count': counts.values})
        counts_df['Class'] = counts_df['Class'].astype(str)
        
        fig = px.pie(counts_df, values='Count', 
        names='Class', 
        title=f'Success vs. Failed counts for {entered_site} Site')
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site, entered_payload):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)',y='class',color="Booster Version Category", 
        title='Payload Mass vs Mission Outcome')
        return fig
